...
public class LeaveSyncService : ILeaveSyncService
{
    ...
    {
        _telemetryEventPublisher.TrackEvent("LeaveSync.Started");
        ...
        _telemetryEventPublisher.TrackEvent("LeaveSync.Completed", new Dictionary<string, string>
        {
            ["LeavesFetched"] = latestLeaves.Count.ToString(),
            ["ImpactedSessions"] = impactedSessionIds.Count.ToString(),
            ["ReassignmentsTriggered"] = reassignments.ToString()
        });
